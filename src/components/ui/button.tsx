...
const buttonVariants = cva(
  'inline-flex items-center justify-center gap-2 whitespace-nowrap rounded-xl text-sm font-semibold ring-offset-background transition-all duration-200 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 active:scale-[0.98] [&_svg]:pointer-events-none [&_svg]:size-4 [&_svg]:shrink-0',
  {
    variants: {
      variant: {
        default:
          'bg-foreground text-background hover:bg-foreground/90 shadow-md hover:shadow-lg hover:-translate-y-0.5',
        destructive:
          'bg-destructive text-destructive-foreground hover:bg-destructive/90 shadow-md hover:shadow-lg',
        outline:
          'border border-border/20 bg-background/50 backdrop-blur-md text-foreground/80 hover:bg-accent hover:text-foreground',
        ...
        'outline-nav':
          'border border-border/10 bg-transparent text-foreground hover:bg-secondary/50 transition-colors',
      },
      size: {
    ...
    return (
      <Comp
        className={cn(
          ...
          isLoading &&
            'scale-[0.98] bg-accent/80 text-accent-foreground animate-pulse pointer-events-none border-transparent shadow-none'
        )}
        ref={ref}
