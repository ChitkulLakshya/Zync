              ...
              {mockProjects.map((project) => (
                ...
                >
                  <CardHeader className="p-3 pb-2">
                    ...
                    <CardTitle className="text-[11px] line-clamp-1">{project.name}</CardTitle>
                  ...
                  <div className="flex items-center gap-2.5">
                    <div className="relative">
                      <Avatar className="w-8 h-8">
                        <AvatarFallback className="text-[9px] bg-foreground/5 text-foreground">
                          {person.avatar}
                        </AvatarFallback>
                      </Avatar>
                      <div
                        className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-background ${
                          person.status === 'online' ? 'bg-task-green' : 'bg-task-yellow'
                        }`}
                      />
                    </div>
              ...
              <Card className="p-2.5 bg-secondary/5 border-border/10 rounded-2xl">
                <div className="grid grid-cols-7 gap-0.5 text-center mb-1">
                  {['S', 'M', 'T', 'W', 'T', 'F', 'S'].map((d, i) => (
                    <div key={i} className="text-[8px] text-muted-foreground font-medium py-0.5">
                      {d}
                    </div>
                  ))}
                </div>
                      ...
                      <div
                        ...
                        className={`aspect-square flex flex-col items-center justify-center rounded text-[9px] ${
                          day < 1 || day > 31
                            ...
                            : isToday
                              ? 'bg-primary text-foreground-foreground font-medium'
                              : 'text-foreground'
                        }`}
                      >
                        {day >= 1 && day <= 31 && (
                          <>
                            {day}
                            {hasEvent && !isToday && (
                              <div className="w-0.5 h-0.5 rounded-full bg-task-orange mt-0.5" />
                            )}
                          </>
                        )}
                      </div>
          ...
          {activeTab === 'tasks' && (
            <div className="p-3 space-y-3">
              ...
              {mockTasks.map((task) => (
                <Card
                  key={task.id}
                  className={`p-2.5 bg-secondary/5 border-border/10 rounded-2xl ${task.done ? 'opacity-60' : ''}`}
                >
                  <div className="flex items-center gap-2">
                    <div
                      className={`w-3.5 h-3.5 rounded border flex items-center justify-center ${
                        task.done ? 'bg-foreground border-foreground' : 'border-muted-foreground/30'
                      }`}
                    >
                      {task.done && <CheckSquare className="w-2 h-2 text-background" />}
                    </div>
                    <span
                      className={`text-[10px] flex-1 ${task.done ? 'line-through text-muted-foreground' : 'text-foreground'}`}
                    >
                      {task.title}
                    </span>
                    <Badge
          ...
          <div className="flex items-center justify-between">
            {[
              ...
              <button
                ...
                className={`flex flex-col items-center justify-center p-1 min-w-[36px] transition-colors ${
      ...
      <AnimatePresence>
        {!hasInteracted && (
          <motion.div
            ...
            className="absolute bottom-[102px] -left-2 sm:-left-20 z-[100] px-4 sm:px-5 py-2 sm:py-2.5 rounded-full bg-surface-glass-regular backdrop-blur-thick border border-black/10 dark:border-white/10 shadow-[0_8px_30px_rgb(0,0,0,0.12)] flex items-center gap-2 pointer-events-none whitespace-nowrap"
          >
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-full bg-blue-400 opacity-75"></span>
              <span className="relative inline-flex rounded-full h-2 w-2 bg-blue-500"></span>
            </span>
            <span className="text-[10px] sm:text-[11px] font-semibold tracking-wide text-foreground/90 shadow-sm">
              Interactive Preview
            </span>
          </motion.div>
        )}
