    ...
    <div className="relative w-full max-w-sm aspect-[4/3] bg-surface-glass-regular backdrop-blur-regular border border-border/10 rounded-xl overflow-hidden shadow-elevation3 mx-auto flex flex-col">
      ...
      <div className="h-10 flex items-center justify-between px-3 border-b border-border/30 bg-secondary/30 relative z-10">
        <div className="flex gap-1.5">
          <div className="px-1.5 py-0.5 rounded border border-border/20 text-[7px] font-medium text-foreground bg-background/50 shadow-sm flex items-center cursor-default hover:bg-accent transition-colors">Today</div>
        ...
        <span className="text-[10px] font-semibold text-foreground">January 2026</span>
        <div className="flex gap-0.5">
          <div className="px-1.5 py-0.5 rounded border border-border/20 text-[7px] font-medium text-primary-foreground bg-primary shadow-sm flex items-center cursor-default">Month</div>
          <div className="px-1.5 py-0.5 rounded border border-border/20 text-[7px] font-medium text-foreground bg-background/50 shadow-sm flex items-center cursor-default hover:bg-accent transition-colors">Week</div>
          <div className="px-1.5 py-0.5 rounded border border-border/20 text-[7px] font-medium text-foreground bg-background/50 shadow-sm flex items-center cursor-default hover:bg-accent transition-colors">Day</div>
        </div>
      </div>
      ...
      <div className="flex flex-col flex-1 relative overflow-hidden bg-background/50 p-2">
        ...
        <div className="grid grid-cols-7 border border-border/10 rounded-md overflow-hidden flex-1 bg-secondary/10 relative z-10" style={{ gridTemplateRows: 'auto repeat(4, 1fr)' }}>
          ...
          {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map(day => (
            <div key={day} className="text-[6px] font-semibold text-muted-foreground text-center py-1 border-b border-border/10 bg-secondary/20">
              {day}
            </div>
          ))}
          ...
          {[...Array(28)].map((_, i) => {
            ...
            return (
              ...
              >
                <div className={`text-[6px] text-right px-1 ${dayNum === 15 ? 'text-primary font-bold' : 'text-muted-foreground'}`}>
                  {dayNum}
                </div>
                
                {/* Dummy events */}
                {i === 2 && (
                   <div className="bg-task-green text-primary-foreground text-[5px] px-1 py-[1px] rounded-[2px] truncate opacity-60 font-medium">
                     Kickoff
                   </div>
                )}
                {i === 18 && (
                   <div className="bg-task-blue text-primary-foreground text-[5px] px-1 py-[1px] rounded-[2px] truncate opacity-60 font-medium">
                     Team Sync
                   </div>
                )}
