...
const UserListItem = ({ user, selectedUser, userStatuses, onSelectUser, isPreview }: any) => {
  const status = !isPreview && userStatuses[user.uid] ? userStatuses[user.uid].status : user.status;
  ...
  return (
    <div
      ...
      className={`flex items-center gap-4 p-3 rounded-xl cursor-pointer transition-all duration-200 group ${
        isSelected ? 'bg-foreground/5 dark:bg-foreground/10' : 'hover:bg-secondary/40'
      }`}
    ...
    >
      ...
      <div className="flex-1 min-w-0 flex flex-col gap-0.5">
        <div
          className={`font-semibold truncate ${isSelected ? 'text-foreground' : 'text-foreground/90'}`}
        >
          {getUserName(user)}
        </div>
              ...
              <div className="mb-2">
                ...
                {myTeamsOpen && (
                  <div className="space-y-1 transition-all duration-300 ease-in-out">
                    ...
                    {myTeams.length === 0 && (
                      <p className="text-xs text-center py-2 text-muted-foreground opacity-50">
                ...
                <div className="flex items-center justify-between px-3 py-2 text-sm font-semibold text-muted-foreground">
                  ...
                  >
                    <span className="flex items-center gap-2">
                      <Star className="w-3.5 h-3.5 fill-current" /> Close Friends
                    </span>
                    {closeFriendsOpen ? (
                      <ChevronDown className="w-4 h-4" />
                    ) : (
                      ...
                      <div className="space-y-2 max-h-[60vh] overflow-y-auto mt-2 pr-1">
                        {users.map((user) => {
                          ...
                          return (
                            <div
                              key={user.uid}
                              className="flex items-center justify-between p-2 rounded-lg hover:bg-secondary/20 border border-transparent hover:border-border/30"
                            >
