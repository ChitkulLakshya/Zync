...
const AvatarLabel = (props: any) => {
  ...
  const user = users.find(
    (u: User) => u.displayName === value || u.email === value || u.uid === value
  );
...
export const ActivityGraph = ({
  ...
  useMemo(() => {
    if (teamSessions.length > 0 && tasks.length === 0) {
      setMode('time');
    }
  }, [teamSessions, tasks]);
    ...
    <Card className="w-full bg-card/50 backdrop-blur-xl border-border/10 shadow-sm rounded-[2rem]">
      <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
        <div className="flex flex-col gap-1">
          ...
          <CardDescription>
            {mode === 'time'
              ? 'Total active hours tracked on platform'
              : 'User activity based on task creation/updates'}
          </CardDescription>
              ...
              <Popover open={openTeamSelect} onOpenChange={setOpenTeamSelect}>
                <PopoverTrigger asChild>
                  <button
                    ...
                    className={cn(
                      'w-[200px] justify-between flex items-center px-3 py-2 text-sm border rounded-md bg-background hover:bg-accent hover:text-accent-foreground',
                      !showTeamOnly && 'opacity-50 cursor-not-allowed'
                    )}
                  >
                    {selectedTeamId
