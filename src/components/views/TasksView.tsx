...
const TasksView = ({ currentUser, users = [] }: TasksViewProps) => {
      ...
      fetchedProjects.forEach((p) => {
        p.steps.forEach((step: any) => {
          (step.tasks || []).forEach((t: any, idx: number) => {
            if (t.assignedTo !== currentUser.uid) {
              return;
            }
  ...
  const markTaskActive = async (task: FlattenedTask) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/api/projects/${projectId}/steps/${stepId}/tasks/${taskId}`,
        {
          method: 'PUT',
          headers: {
      ...
      toast({
        title: 'Update Failed',
        description: 'Could not move task to Active.',
        variant: 'destructive',
      });
    }
  };
  ...
  const handleOpenArchitecture = async (task: FlattenedTask) => {
    await markTaskActive(task);
    navigate(`/dashboard/workspace/project/${task.projectId}`, {
      state: { from: '/dashboard/tasks' },
    });
  };
      ...
      {groupedTasks.length === 0 ? (
        <div className="flex flex-col items-center justify-center flex-1 text-center px-4">
          ...
          <p className="text-sm text-muted-foreground mt-2 max-w-sm leading-relaxed">
            You have no tasks assigned at the moment. When a project owner assigns you work, it will
            appear here.
          </p>
        </div>
      ) : (
                  ...
                  {group.tasks.map((task) => (
                    ...
                    >
                      <div className="flex items-start gap-3 mb-4">
                        <div
                          className={`w-2.5 h-2.5 rounded-full mt-1.5 shrink-0 ${getStatusColor(task.status)}`}
                        />
