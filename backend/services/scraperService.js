...
const LAUNCH_OPTIONS = {
  headless: 'new',
  args: ['--disable-gpu', '--disable-dev-shm-usage', '--window-size=1600,1200'],
};

const SHARED_BROWSER_IDLE_MS = Number.parseInt(
  process.env.SHARED_BROWSER_IDLE_MS || '300000',
  10
);
...
async function scrapeSiteInspire(browser, query) {
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('.WebsiteCard'))
        .map((card) => {
          const externalLink = card.querySelector('a.ExternalLinkButton');
          const internalLink = card.querySelector(
            'a.WebsiteCard__imageWrapper'
          );
          const img =
            card.querySelector('img.WebsiteCard__image') ||
...
async function scrapeDribbble(browser, query) {
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      ...
      items.forEach((item) => {
        const linkEl =
          item.querySelector('a.shot-thumbnail-link') ||
          item.querySelector('a');
        const titleEl = item.querySelector('div.shot-title');
        const imgEl =
          item.querySelector('figure img') || item.querySelector('img');
...
async function scrapeAwwwards(browser, query) {
  ...
  try {
    ...
    const results = await page.evaluate(() => {
      return Array.from(document.querySelectorAll('.card-site'))
        .map((card) => {
