...
router.get('/user/:uid', verifyToken, async (req, res) => {
  ...
  if (req.user.uid !== uid) {
    return res
      .status(403)
      .json({ message: 'Unauthorized access to meeting history' });
  }
  ...
  try {
    ...
    const { items, pagination } = paginateArray(updatedMeetings, req.query, {
      defaultLimit: 20,
      maxLimit: 100,
    });
...
router.delete('/:meetingId', verifyToken, async (req, res) => {
  ...
  try {
    ...
    if (meeting.organizerId !== uid) {
      return res
        .status(403)
        .json({ message: 'Only the organizer can delete this meeting' });
    }
...
router.post('/schedule', verifyToken, async (req, res) => {
  ...
  try {
    ...
    (async () => {
      await Promise.all(
        participants.map(async (receiver) => {
          if (receiver.email) {
            try {
...
router.post('/invite', verifyToken, async (req, res) => {
  ...
  try {
    ...
    const receivers =
      Array.isArray(receiverIds) && receiverIds.length > 0
          ...
          receivers.map(async (receiver) => {
            if (receiver.email) {
              try {
                ...
                const recipientName =
                  receiver.displayName || receiver.firstName || 'there';
