...
if (
  ...
  console.warn(
    '[verifyGithub] GITHUB_WEBHOOK_SECRET is unset; GitHub webhook routes will return 500 until set.'
  );
}
...
const verifyGithub = (req, res, next) => {
  try {
    ...
    if (!req.rawBody) {
      console.error(
        'req.rawBody is missing. Ensure json parser verify option is set.'
      );
