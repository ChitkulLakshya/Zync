...
async function migrate() {
  ...
  try {
    ...
    for (const team of mongoTeams) {
      ...
      if (!ownerId) {
        console.warn(
          `Skipping team ${team.name}: Owner ${team.ownerId} not found`
        );
        continue;
      }

      const members = (team.members || [])
        .map((m) => idMap.get(m?.toString()))
        .filter(Boolean);
    ...
    for (const project of mongoProjects) {
      ...
      if (!ownerId && !teamId) {
        console.warn(
          `Skipping project ${project.name}: No valid Owner or Team found`
        );
        continue;
      }

      const collaborators = (project.collaborators || [])
        .map((c) => idMap.get(c?.toString()))
        .filter(Boolean);
