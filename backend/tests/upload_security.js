...
async function run() {
  ...
  try {
    ...
    const res = await request(app).post('/api/upload').attach('file', buffer, {
    ...
    if (res.status === 200) {
      if (res.body.fileUrl.endsWith('.txt')) {
        ...
        console.error(
          '❌ Failed: File saved with extension:',
          path.extname(res.body.fileUrl)
        );
        passed = false;
      }
    } else {
  ...
  try {
    ...
    if (res.status === 200) {
      if (res.body.fileUrl.endsWith('.png')) {
        ...
        console.error(
          '❌ Failed: Valid PNG saved with wrong extension:',
          path.extname(res.body.fileUrl)
        );
        passed = false;
      }
  ...
  try {
    ...
    if (res.status === 200) {
      if (res.body.fileUrl.endsWith('.png')) {
        console.log(
          '✅ Passed: Spoofed file saved with .png extension (harmless).'
        );
      } else {
        console.error(
          '❌ Failed: Spoofed file saved with dangerous extension:',
          path.extname(res.body.fileUrl)
        );
        passed = false;
      }
