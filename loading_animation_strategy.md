...
- **The Interaction:** When a user clicks a button, it scales to `0.98` (the `press` preset). The button's background transitions to a subtle, muted, repeating opacity pulse. We apply the `color.accent.default` (muted/desaturated) _only_ to this active state, drawing the eye gently without vibrating against the dark background.
